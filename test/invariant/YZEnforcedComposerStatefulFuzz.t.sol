...
contract YZEnforcedComposerStatefulFuzzTest is YZEnforcedComposerBase {
    ...
    function _fuzzDeposit() internal {
        ...
        _prepareExpectRevert(user, amount);
        vm.prank(user);
    ...
    function _fuzzRedeem() internal {
        ...
        if (shares > 0) {
            ...
            _prepareExpectRedeemRevert(user, amount);
            vm.prank(user);
            try yzEnforcedComposer_arb.redeemAndSend(amount, redeemParam, user) {
    ...
    function _fuzzUserDeposit(address user) internal {
        ...
        _prepareExpectRevert(user, amount);
        vm.prank(user);
        try yzEnforcedComposer_arb.depositAndSend(amount, sendParam, user) {} catch {}
    }
    ...
    function _fuzzUserRedeem(address user) internal {
        ...
        if (shares > 0) {
            ...
            _prepareExpectRedeemRevert(user, amount);
            vm.prank(user);
            try yzEnforcedComposer_arb.redeemAndSend(amount, redeemParam, user) {} catch {}
        }
    }
    ...
    function _fuzzCapBoundaryTesting() internal {
        ...
        _prepareExpectRevert(user, cap);
        vm.prank(user);
        try yzEnforcedComposer_arb.depositAndSend(cap, sendParam, user) {} catch {}
    }
    ...
    function _fuzzCapRaceConditions() internal {
        ...
        _prepareExpectRevert(user1, amount1);
        vm.prank(user1);
        try yzEnforcedComposer_arb.depositAndSend(amount1, sendParam1, user1) {} catch {}

        _prepareExpectRevert(user2, amount2);
        vm.prank(user2);
        try yzEnforcedComposer_arb.depositAndSend(amount2, sendParam2, user2) {} catch {}
    }
        ...
        if (yzEnforcedComposer_arb.whitelistEnabled()) {
            for (uint256 i = 0; i < users.length; i++) {
                ...
                if (isWhitelisted) {
                    ...
                    _prepareExpectRevert(users[i], amount);
                    vm.prank(users[i]);
                    try yzEnforcedComposer_arb.depositAndSend(amount, sendParam, users[i]) {} catch {}
                } else {
                    // Should not be able to deposit
                    uint256 amount = 1 ether;
                    _fundLocalFromHub(users[i], amount);
    ...
    function _getFuzzUsers() internal pure returns (address[] memory) {
        address[] memory users = new address[](5);
        users[0] = address(uint160(uint256(keccak256(abi.encodePacked("fuzzUser", uint256(0))))));
        users[1] = address(uint160(uint256(keccak256(abi.encodePacked("fuzzUser", uint256(1))))));
        users[2] = address(uint160(uint256(keccak256(abi.encodePacked("fuzzUser", uint256(2))))));
        users[3] = address(uint160(uint256(keccak256(abi.encodePacked("fuzzUser", uint256(3))))));
        users[4] = address(uint160(uint256(keccak256(abi.encodePacked("fuzzUser", uint256(4))))));
        return users;
    }
    ...
    function _prepareExpectRevert(address user, uint256 amount) internal returns (bool willRevert) {
        ...
        if (yzEnforcedComposer_arb.whitelistEnabled() && !yzEnforcedComposer_arb.whitelist(user)) {
            vm.expectRevert(abi.encodeWithSelector(YZEnforcedComposer.YZ_NotWhitelisted.selector, user));
            return true;
        }
        uint256 currentTVL = vault_arb.totalAssets();
        uint256 tvlCap = yzEnforcedComposer_arb.tvlCap();
        if (tvlCap > 0 && currentTVL + amount > tvlCap) {
            vm.expectRevert(
                abi.encodeWithSelector(YZEnforcedComposer.YZ_TVLCapExceeded.selector, currentTVL, amount, tvlCap)
            );
            return true;
        }
        ...
        if (userCap > 0 && currentUserDeposit + amount > userCap) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    YZEnforcedComposer.YZ_UserCapExceeded.selector, user, currentUserDeposit, amount, userCap
                )
            );
            return true;
        }
    ...
    function _prepareExpectRedeemRevert(address user, uint256 shares) internal returns (bool willRevert) {
        if (yzEnforcedComposer_arb.redemptionsPaused()) {
            vm.expectRevert(YZEnforcedComposer.YZ_RedemptionsPaused.selector);
            return true;
        }
        uint256 userShares = vault_arb.balanceOf(user);
