...
contract YZEnforcedComposerFormalInvariants is YZEnforcedComposerBase {
    ...
    uint256 internal constant MAX_SHARE_SUPPLY = 1e26;
    uint256 internal constant MAX_ASSET_AMOUNT = 1e26;
    ...
    function test_StorageAtomicity_SourceStateRollback() public {
        ...
        // Should handle failure gracefully (expect revert)
        vm.expectRevert();
        vm.prank(userA);
    ...
    function test_MalformedPayload_Fuzz() public {
        ...
        for (uint256 i = 0; i < malformedPayloads.length; i++) {
            ...
            assertGe(vault_arb.totalAssets(), 0, "Assets corrupted");
    ...
    function _testMalformedPayload(bytes memory payload) external {
        ...
        SendParam memory invalidParam = SendParam({
            dstEid: 0, to: bytes32(0), amountLD: 0, minAmountLD: 0, extraOptions: "", composeMsg: payload, oftCmd: ""
        });
    ...
    function test_AdminGriefing_WhitelistRemovalDuringExecution() public {
        ...
        // Start deposit: simulate receipt of assets by the composer from cross-chain message
        _fundLocalFromHub(address(yzEnforcedComposer_arb), 50 ether);
        ...
        // Deposit should revert since user is no longer whitelisted
        ...
        vm.prank(address(yzEnforcedComposer_arb));
        yzEnforcedComposer_arb.handleCompose(
            address(assetOFT_arb), OFTComposeMsgCodec.addressToBytes32(userA), composeMsg, 50 ether
        );
        ...
        vm.expectRevert(abi.encodeWithSelector(YZEnforcedComposer.YZ_NotWhitelisted.selector, userB));
        vm.prank(userB);
    ...
    function test_AdminGriefing_PauseDuringCompose() public {
        ...
        // Start deposit process: simulate receipt of assets by the composer from cross-chain message
        _fundLocalFromHub(address(yzEnforcedComposer_arb), 50 ether);
        ...
        // Deposit should revert since pause blocks cross-chain compose execution
        bytes memory composeMsg = abi.encode(sendParam, uint256(0));
        vm.expectRevert(YZEnforcedComposer.YZ_DepositsPaused.selector);
        vm.prank(address(yzEnforcedComposer_arb));
        yzEnforcedComposer_arb.handleCompose(
