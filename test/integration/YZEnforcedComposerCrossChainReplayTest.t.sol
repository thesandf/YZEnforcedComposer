...
contract YZEnforcedComposerCrossChainReplayTest is YZEnforcedComposerBase {
    ...
    function test_CrossChainReplay_DifferentChain_NotVulnerable() public {
        ...
        // User deposits from ETH chain (simulated)
        _fundLocalFromHub(userA, 50 ether);
        vm.prank(userA);
        assetOFT_arb.approve(address(yzEnforcedComposer_arb), 50 ether);
    ...
    function test_CrossChainReplay_UserTracking_AccurateAcrossChains() public {
        ...
        vm.prank(userA);
        yzEnforcedComposer_arb.depositAndSend{value: fee1}(60 ether, sendParam, userA);

        assertEq(yzEnforcedComposer_arb.userDeposits(userA), 60 ether);

        // User B deposits from POL
        _fundLocalFromHub(userB, 50 ether);
        vm.prank(userB);
        assetOFT_arb.approve(address(yzEnforcedComposer_arb), 50 ether);

        sendParam = _buildHopParam(address(0), userB, POL_EID, 50 ether);
        uint256 fee2 = _getAndFundDepositFee(userB, sendParam);

        vm.prank(userB);
        yzEnforcedComposer_arb.depositAndSend{value: fee2}(50 ether, sendParam, userB);

        assertEq(yzEnforcedComposer_arb.userDeposits(userB), 50 ether);
