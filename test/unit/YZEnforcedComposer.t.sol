...
contract YZEnforcedComposerUnitTest is YZEnforcedComposerBase {
    ...
    function test_Deposit_RevertsWhenTVLCapExceeded() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(YZEnforcedComposer.YZ_TVLCapExceeded.selector, 0, depositAmount, tvlCap));
        vm.prank(userA);
    ...
    function test_Deposit_RevertsWhenUserCapExceeded() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(YZEnforcedComposer.YZ_UserCapExceeded.selector, userA, 0, depositAmount, userCap)
        );
        vm.prank(userA);
        yzEnforcedComposer_arb.depositAndSend(depositAmount, sendParam, userA);
    ...
    function test_SetTVLCap_UpdatesCapAndEmitsEvent() public {
        ...
        emit YZEnforcedComposer.TVLCapUpdated(MAX_TEST_AMOUNT, newCap);
    ...
    function test_EmergencyWithdraw_Assets_RevertsOnInsufficientBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(YZEnforcedComposer.YZ_InsufficientBalance.selector, 0, withdrawAmount));
        vm.prank(admin);
        yzEnforcedComposer_arb.emergencyWithdraw(withdrawAmount, recipient);
    }
    ...
    function test_EmergencyWithdraw_Shares_TransfersCorrectly() public {
        uint256 depositAmount = 100 ether;
        _fundLocalFromHub(address(yzEnforcedComposer_arb), depositAmount);

        vm.prank(address(yzEnforcedComposer_arb));
        assetOFT_arb.approve(address(vault_arb), depositAmount);

        vm.prank(address(yzEnforcedComposer_arb));
        vault_arb.deposit(depositAmount, address(yzEnforcedComposer_arb));
