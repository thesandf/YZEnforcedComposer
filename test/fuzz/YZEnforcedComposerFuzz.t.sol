...
contract YZEnforcedComposerFuzzTest is YZEnforcedComposerBase {
    ...
    function testFuzz_Deposit_RevertsWhenTVLCapExceeded(uint256 _tvlCap, uint256 _depositAmount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(YZEnforcedComposer.YZ_TVLCapExceeded.selector, 0, _depositAmount, _tvlCap)
        );
        vm.prank(userA);
    ...
    function testFuzz_Deposit_RevertsWhenUserCapExceeded(uint256 _userCap, uint256 _depositAmount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(YZEnforcedComposer.YZ_UserCapExceeded.selector, userA, 0, _depositAmount, _userCap)
        );
        vm.prank(userA);
        yzEnforcedComposer_arb.depositAndSend(_depositAmount, sendParam, userA);
    ...
    function testFuzz_BatchSetUserCaps_ValidInputs(address[5] memory _users, uint256[5] memory _caps) public {
    ...
    function testFuzz_BatchSetWhitelist_ValidInputs(address[5] memory _users, bool[5] memory _statuses) public {
        // Filter out zero addresses
        address[] memory validUsers = new address[](5);
    ...
    function testFuzz_CanDeposit_ConsistentWithActualDeposit(uint256 _userCap, uint256 _tvlCap, uint256 _testAmount)
        public
    {
        _userCap = bound(_userCap, MIN_CAP, MAX_FUZZ_AMOUNT);
    ...
    function testFuzz_EmergencyWithdraw_Assets_RevertsOnInsufficientBalance(uint256 _withdrawAmount) public {
        ...
        vm.expectRevert(abi.encodeWithSelector(YZEnforcedComposer.YZ_InsufficientBalance.selector, 0, _withdrawAmount));
        vm.prank(admin);
        yzEnforcedComposer_arb.emergencyWithdraw(_withdrawAmount, recipient);
    }
