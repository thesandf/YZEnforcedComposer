...
contract YZEnforcedComposerInflationAttackTest is YZEnforcedComposerBase {
    ...
    function test_InflationAttack_AssetSupply_NotVulnerable() public {
        ...
        vm.prank(userA);
        vm.expectRevert();
        assetOFT_arb.mint(userA, 100 ether); // Should fail - only admin can mint
    ...
    function test_InflationAttack_TVLManipulation_NotVulnerable() public {
        ...
        vm.prank(userA);
        vm.expectRevert();
        vault_arb.setTotalAssets(200 ether); // Should fail - only vault controls TVL
    ...
    function test_InflationAttack_UserDepositTracking_NotVulnerable() public {
        ...
        vm.prank(userA);
        vm.expectRevert();
        yzEnforcedComposer_arb.exposed_setUserDeposit(userA, 10 ether); // Should fail - not admin
    ...
    function test_InflationAttack_SharePriceManipulation_NotVulnerable() public {
        ...
        vm.prank(userA);
        vm.expectRevert();
        vault_arb.setPricePerShare(2 ether); // Should fail - only vault controls price
