...
contract YZEnforcedComposerEconomicSimulation is YZEnforcedComposerBase {
    ...
    function test_EconomicSimulation_WhaleSandwiching() public {
        ...
        _setupUserCap(victim, 50 ether);
        _setupTVLCap(1000 ether);
        
        // Set initial TVL close to cap
        _executeSuccessfulDeposit(victim, 900 ether);
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            vm.expectRevert(YZEnforcedComposer.YZ_TVLCapExceeded.selector);
            vm.prank(whale);
            ...
            // Check for sandwich attack success
            _checkSandwichAttackPrevention();
        }
    }
    ...
    function test_EconomicSimulation_CapEdgeGriefing() public {
        ...
        for (uint256 i = 0; i < 100; i++) {
            ...
            // Check for griefing success
            _checkCapEdgeGriefing();
        }
    }
    ...
    function test_EconomicSimulation_GasGriefingCrossChain() public {
        ...
        _setupUserCap(victim, 100 ether);
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            vm.prank(admin);
            yzEnforcedComposer_arb.setTVLCap((i % 2 == 0) ? 1500 ether : 2500 ether);
    ...
    function test_AdversarialEconomic_WhaleFrontRunning() public {
        ...
        // Whale front-runs with larger deposit
        _fundLocalFromHub(whale, 950 ether);
        vm.prank(whale);
        assetOFT_arb.approve(address(yzEnforcedComposer_arb), 950 ether);
        
        SendParam memory whaleSendParam = _buildHopParam(address(0), whale, ARB_EID, 950 ether);
        
        // Whale should succeed, small user should fail due to cap
        vm.prank(whale);
        yzEnforcedComposer_arb.depositAndSend(950 ether, whaleSendParam, whale);
        ...
        assertEq(vault_arb.totalAssets(), 950 ether, "Cap not enforced correctly");
    }
    ...
    function test_AdversarialEconomic_RetryManipulation() public {
        ...
        SendParam memory sendParam = _buildHopParam(address(0), user, ARB_EID, 500 ether);
        
        // First attempt should fail
        vm.prank(user);
        yzEnforcedComposer_arb.depositAndSend(500 ether, sendParam, user);
    ...
    function _checkEconomicDegradation(uint256 cycle) internal {
        ...
        // Check for TVL degradation
        if (cycle > 0) {
            assertGe(currentTVL, economicState.netTVL, "TVL degradation detected");
        }
    ...
    function _finalEconomicHealthCheck() internal {
        ...
        if (totalOperations > 0) {
            ...
            assertGe(successRate, 50, "Success rate too low - economic degradation");
        }
    }
    ...
    function _checkCapEdgeGriefing() internal {
        ...
        if (tvlCap > 0 && currentTVL > tvlCap) {
            assert(false, "Cap edge griefing succeeded");
        }
    }
    ...
    function _simulateDeposit(address user, uint256 amount) internal {
        ...
        try vm.prank(user) yzEnforcedComposer_arb.depositAndSend(amount, sendParam, user) {
            economicState.successfulOperations++;
    ...
    function _simulateRedeem(address user, uint256 amount) internal {
        ...
        try vm.prank(user) yzEnforcedComposer_arb.redeemAndSend(redeemShares, redeemParam, user) {
            economicState.successfulOperations++;
